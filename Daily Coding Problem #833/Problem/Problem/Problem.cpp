...
class Person
{
    ...
    int* knows;
    public:
        Person(int id, int knows[])
        {
            this->id = id;
            this->knows = new int[sizeof(knows) / sizeof(int)];
            for (int i = 0; i < sizeof(knows) / sizeof(int); i++)
            {
                this->knows[i] = knows[i];
            }
        }
};
...
{
    int N;
    cout << "Fetch a list with(people):";
    cin >> N;

}

bool knows(Person a, Person b)
{
